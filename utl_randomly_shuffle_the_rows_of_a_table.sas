...
see  Paul's Hash solution on end
Paul Dorfman
...
OTOH, advantage can be taken of the fact that the items in a hash object instance are stored randomly by their key-values, so we can simply "extrude" the data set through the hash:
